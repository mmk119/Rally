...
- **Booking tab** — a four step flow beside a **live summary rail** that fills in as you choose, so the booking is reviewable at every step: pick **one or several days** (click a chosen day again to clear it) and an hourly slot (slots grouped into morning / afternoon / evening, with selected, open, and full states), choose a court, add details, confirm. Confirmation shows a booking reference with an animated check. Your bookings are listed below with a **cancel** action.
  **Block booking:** selecting several days applies the same court and time to each. Availability is the intersection — a slot only offers a court that is free on *every* selected day — and confirming creates one reservation per day, each with its own reference so it can be cancelled on its own.
